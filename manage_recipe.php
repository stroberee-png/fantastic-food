...
/* =========================
   PAGINATION SETUP
========================= */
$recipesPerPage = 5; // recipes per page
...
if (isset($_GET['action'], $_GET['id'])) {
    ...
    header("Location: manage_recipe.php?page=$page");
    exit;
}
...
========================= */
$totalRecipesResult = mysqli_query($conn, "SELECT COUNT(*) as total FROM recipes");
$totalRecipesData = mysqli_fetch_assoc($totalRecipesResult);
$totalRecipes = $totalRecipesData['total'];
$totalPages = ceil($totalRecipes / $recipesPerPage);

/* =========================
   FETCH RECIPES WITH LIMIT
========================= */
$recipes = [];
            ...
            <tr>
                ...
                <td class="actions">
                    ...
                    <?php if ($r['status'] === 'Pending'): ?>
                        <a href="?action=approve&id=<?= $r['recipe_id'] ?>&page=<?= $page ?>" class="approve">Approve</a>
                        <a href="?action=reject&id=<?= $r['recipe_id'] ?>&page=<?= $page ?>" class="reject">Reject</a>
                    <?php endif; ?>
                    ...
                    <?php if ($r['status'] === 'Approved'): ?>
                        ...
                        <?php if (!$r['is_featured']): ?>
                            <?php if ($featuredCount < 3): ?>
                                <a href="?action=feature&id=<?= $r['recipe_id'] ?>&page=<?= $page ?>" class="feature">Feature</a>
                            <?php else: ?>
                                <span class="feature disabled">Feature</span>
                            <?php endif; ?>
                        <?php else: ?>
                            <a href="?action=unfeature&id=<?= $r['recipe_id'] ?>&page=<?= $page ?>" class="unfeature">Unfeature</a>
                        <?php endif; ?>

                    <?php endif; ?>

                    <a href="?action=delete&id=<?= $r['recipe_id'] ?>&page=<?= $page ?>"
                        class="delete"
