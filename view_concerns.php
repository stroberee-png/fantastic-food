...
if (isset($_GET['resolve']) && is_numeric($_GET['resolve'])) {
    ...
    header("Location: view_concerns.php?page=$page");
    exit;
}
...
/* =========================
   COUNT TOTAL CONCERNS (for pagination)
========================= */
$totalConcernsResult = mysqli_query($conn, "SELECT COUNT(*) AS total FROM concerns");
$totalConcernsData = mysqli_fetch_assoc($totalConcernsResult);
$totalConcerns = $totalConcernsData['total'];
$totalPages = ceil($totalConcerns / $concernsPerPage);

/* =========================
   FETCH CONCERNS WITH LIMIT
========================= */
$sql = "
SELECT 
    c.concern_id,
...
<td>
<div class="message-preview"><?= htmlspecialchars($c['message']) ?></div>
<br>
...
<td>
<span class="status <?= strtolower($c['status']) ?>"><?= $c['status'] ?></span>
</td>
<td>
<?php if ($c['status'] === 'Pending'): ?>
<a class="btn btn-resolve" href="?resolve=<?= $c['concern_id'] ?>&page=<?= $page ?>">Resolve</a>
<?php else: ?>
—
<?php endif; ?>
</td>
